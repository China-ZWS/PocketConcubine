...
- (id)initWithType:(NavBarButtonItemType)itemType image:(NSString *)image title:(NSString *)title
{
    ...
    if (self)
    {
        ...
        CGSize titleSize = [self createTitle:title font:Font(15) maxSize:CGSizeMake(100, Font(15).lineHeight)];
    ...
    if ((self = [super init])) {
        if (images.count) {
            ...
            for (int i = 0; i < images.count; i ++)
            {
                ...
                CGSize imageSize = img.size;
                CGSize titleSize = [self createTitle:titles[i] font:Font(15) maxSize:CGSizeMake(100, Font(15).lineHeight)];
                UIButton *button = [UIButton buttonWithType:UIButtonTypeCustom];
                button.frame = CGRectMake(0, 0, imageSize.width + titleSize.width, (img.size.height - titleSize.height > 0?imageSize.height:titleSize.height));
                button.titleLabel.font = Font(15);
