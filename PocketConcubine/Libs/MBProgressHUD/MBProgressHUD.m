...
- (void)layoutSubviews {
	...
	label.frame = labelF;
#warning !!!
    if([self.MBprogressType isEqualToString:@"Test"]){
        CGFloat x=25;
        self.frame =CGRectMake(label.frame.origin.x-x, label.frame.origin.y-x, label.frame.size.width+x*2, label.frame.size.height+x*2);
        label.frame=CGRectMake(x, x, label.frame.size.width, label.frame.size.height);
    }
